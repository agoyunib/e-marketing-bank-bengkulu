...
@section('content')
    <section class="panel" style="margin-bottom:20px;">
        ...
        <div class="panel-body" style="border-top: 1px solid #eee; padding:15px; background:white;">
            <form action="{{ route('administrator.user_pimpinan.post') }}" method="POST">
                {{ csrf_field() }} {{ method_field('POST') }}
                <div class="row">
                    <div class="col-md-12">
                        <div class="alert alert-info alert-block">
                            <i class="fa fa-info-circle"></i>&nbsp; Silahkan tambahkan pimpinan jika diperlukan.
                        </div>
                    </div>
                   ...
                   <table class="table table-hover table-bordered" id="table">
                       ...
                       <tbody>
                           @php
                               $no=1;
                           @endphp
                           @foreach ($user_pimpinans as $user_pimpinan)                  
                                 <tr>
                                   <td>{{ $no++ }}</td>
                                   <td>{{ $user_pimpinan->no_nrpp }}</td>
                                   ...
                                   <td>
                                    @if ($user_pimpinan->status_user == "1")
                                        <form action="{{ route('administrator.user_pimpinan.non_aktifkan_status', [$user_pimpinan->id]) }}" method="POST">
                                            {{ csrf_field() }} {{ method_field('PATCH') }}
                                            <button type="submit" class="btn btn-danger btn-sm" style="color:white; cursor:pointer;"><i class="fa fa-thumbs-down"></i></button>
                                        </form>
                                        @else
                                        <form action="{{ route('administrator.user_pimpinan.aktifkan_status', [$user_pimpinan->id]) }}" method="POST">
                                            {{ csrf_field() }} {{ method_field('PATCH') }}
